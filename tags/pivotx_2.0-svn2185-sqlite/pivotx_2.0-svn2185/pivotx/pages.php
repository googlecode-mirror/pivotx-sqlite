...
//
// $Id: pages.php 2185 2009-10-22 15:09:06Z pivotlog $
//
// ---------------------------------------------------------------------------
...
function pageLogin($template="normal") {
    ...
    $template = get_default($_POST['template'], $template);

    $form = getLoginForm($template);

    // If a 'return to' is set, pass it onto the template, but only the 'path'
    // part. This means that we do NOT allow redirects to another domain!!
    $returnto = get_default($_GET['returnto'], $_POST['returnto']);
    if (!empty($returnto)) {        
        $returnto = parse_url($returnto);
        $form->setvalue('returnto', $returnto['path']);
    }
    ...
    if ( $result != FORM_OK ) {
        ...
        if ($val['resetpassword'] == 1) {
            ...
            if (!empty($returnto['path'])) {
                header("location: " . $returnto['path'] );
                die();
            } else {      
                if ($template=="normal") {       
                    pageDashboard();
            ...
            $PIVOTX['events']->add('failed_login', "", safe_string($_POST['username']));
...
function pageSetupUser() {
    ...
    if ($PIVOTX['users']->count()!=0) {
        // if there are, display the User setup screen.
        pageLogin();
        die();
    }
    ...
    // Euhm, the following shouldn't be here, i think - Bob..
    // $message[] = "No users were found. Please set up the first Admin user.";
    ...
    if ( $result != FORM_OK ) {
        ...
        $PIVOTX['messages']->addMessage(__("The user has been added! You can login with your new account now."));
...
function pageExtensions() {
    ...
    if ( $result != FORM_OK ) {
        ...
        $PIVOTX['extensions']->setActivated($activated, $filenames);

        $PIVOTX['events']->add('activate_extensions', $activated);
        $PIVOTX['messages']->addMessage(__("Your Extension settings have been stored."));
        $PIVOTX['template']->assign("form", $form->fetch());
...
function pageWidgets() {
    ...
    if ( isset($_GET['widget']) )  {
        ...
        $active = array_merge($active_snippets, $active_hooks, $active_admin, $_GET['widget']);
    ...
    foreach ($list['widget'] as $extension) {
        ...
        $description = sprintf("<li class=\"widget\" id=\"widget_%s\"><strong>%s</strong><br />%s</p><p style='color: #999; margin: 0; font-size: 11px;'>By: %s",
            $extension['name'],
            $extension['extension'],
...
function pageEntry() {
    ...
    if (!isset($_POST['title'])) {
        ...
        if($PIVOTX['config']->get('smarty_cache')){
            $myweblogs = $PIVOTX['weblogs']->getWeblogsWithCat($entry['category']);
            if(is_array($myweblogs)) {
                foreach($myweblogs as $myweblog) {
                    $myweblog = $PIVOTX['weblogs']->getWeblog($myweblog);
                    $PIVOTX['template']->clear_cache(basename($myweblog['front_template']));
                }
            }
            $PIVOTX['template']->clear_cache("_e".$entry['uid']."_");
        }
...
function pagePage() {
    ...
    if ($_POST['title']=="") {
        ...
        // Remove the frontpages and entrypages from the cache.
        if($PIVOTX['config']->get('smarty_cache')){
            clearSmartyCache($page['template']);
            clearSmartyCache("_p".$page['uid']."_");
        }
        ...
        if (($_POST['postedfrom']=="continue") ) {
            ...
            $PIVOTX['events']->add('save_page', intval($page['uid']), $page['title']); 
...
function pagePagesoverview() {
    ...
    $PIVOTX['template']->assign('pages', $PIVOTX['pages']->getIndex(true) );
    $PIVOTX['template']->assign('users', $PIVOTX['users']->getUserNicknames() );
...
function pageTrackbacks() {
    ...
    if ($_GET['uid']!="") {
        ...
        // TODO: Check if the user is allowed to edit this entry..
        ...
        $PIVOTX['template']->assign('heading', __('Edit or Delete Trackbacks in the Last Trackbacks'));
...
function pageComments() {
    ...
    if ($_GET['uid']!="") {
        ...
        $PIVOTX['template']->assign('heading', __('Edit or Delete Comments in the Moderation Queue and Latest Comments'));
...
function pageEditcomment() {
    ...
    if (($_GET['uid']!="") && ($_GET['key']!="")) {
        ...
        // TODO: Check if the user is allowed to edit this comment..
...
function pageEdittrackback() {
    ...
    if (($_GET['uid']!="") && ($_GET['key']!="")) {
        ...
        // TODO: Check if the user is allowed to edit this trackback..
...
function pageWeblogs() {
    ...
    if ( (isset($_GET['del'])) && ($_GET['pivotxsession']==$_COOKIE['pivotxsession']) ) {
        ...
        $PIVOTX['events']->add('delete_weblog', "", safe_string($_GET['del'])); 
    }
...
function pageBackup() {
    ...
    $form_html = $form_backup_cfg->fetch() . $form_backup_templates->fetch();
    if ($PIVOTX['config']->get('db_model')=="flat") {
        $form_backup_database = getBackupDatabaseForm();
        $form_html .= $form_backup_database->fetch();
    } else {
...
/**
 * Display 'backupconfig' page.
 */
function pageBackupconfig() {
    global $PIVOTX;

    // check if the user has the required userlevel to view this page.
    $PIVOTX['session']->minLevel(PIVOTX_UL_NORMAL);

    $PIVOTX['template']->assign('title', __('Backup of Configuration Files'));
    $PIVOTX['template']->assign('heading', __('This will let you download a zip file containing your configuration files'));
...
/**
 * Display 'Backupdb' page.
 */
function pageBackupdb() {
    global $PIVOTX;

    // check if the user has the required userlevel to view this page.
    $PIVOTX['session']->minLevel(PIVOTX_UL_NORMAL);

    $PIVOTX['template']->assign('title', __('Backup of Database'));
    $PIVOTX['template']->assign('heading', __('This will let you download a zip file containing your database (with all your entries)'));
...
function pageUsers() {
    ...
    if ( (isset($_GET['del'])) && ($_GET['pivotxsession']==$_COOKIE['pivotxsession']) ) {
        ...
        if ( ($currentuser['userlevel'] == PIVOTX_UL_SUPERADMIN) ||
            ...
            $PIVOTX['events']->add('delete_user', "", safe_string($_GET['del'])); 
...
function pageUseredit() {
    ...
    if (!empty($_POST['username'])) {
        $_POST['username'] = strtolower(safe_string($_POST['username']));
    }
    ...
    if ( $result == FORM_NOTPOSTED ) {
        ...
        $val = $form->getvalues();
        $val['username'] = strtolower(safe_string($val['username']));
    ...
    openDialog(\"$title\", \"$link\", 540, 520 , $vars);
        ...
        if ($user) {
            ...
            if ($error) {
                ...
                if ($email_taken) {
                    $message = __('E-mail already in use');
                    $errorfield = 'email';
                ...
                $PIVOTX['events']->add('edit_user', "", safe_string($_GET['user'])); 
            ...
            if ($error) {
                ...
                $PIVOTX['events']->add('add_user', "", safe_string($val['username']));
...
function pageCategoryedit() {
    ...
    openDialog(\"$title\", \"$link\", 460, 440 , $vars);
        ...
        if ($_GET['cat']=="") {
            ...
            $cat['name'] = safe_string($cat['name'], true);
...
jQuery(function($) {
    openDialog(\"$title\", \"$link\", 460, 440 , $vars);
        ...
        $PIVOTX['events']->add('edit_category', "", safe_string($cat['name']));
...
function pageBookmarklet() {
    ...
    if (!empty($_GET['title'])) {
        ...
        $entry['introduction'] .= sprintf("<p><a href='%s'>%s</a></p>", $_GET['url'], $_GET['title'] );
    } else {
        $entry['introduction'] .= sprintf("<p><a href='%s'>%s</a></p>", $_GET['url'], $_GET['title'] );      
    }

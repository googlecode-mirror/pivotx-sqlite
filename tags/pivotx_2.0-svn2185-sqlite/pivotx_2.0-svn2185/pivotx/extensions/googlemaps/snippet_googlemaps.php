<?php
// - Extension: Google Maps snippet
// - Version: 1.0 - alpha 1
// - Author: PivotX Team
// - Email: admin@pivotx.net
// - Site: http://www.pivotx.net
// - Updatecheck: http://www.pivotx.net/update.php?ext=googlemaps
// - Description: Insert a Google Maps with an KML overlay. [<a href="extensions/googlemaps/">docs</a>]
// - Date: 2008-06-26
...
function smarty_googlemaps($params, &$smarty) {
    ...
    if (!$initialized) {
        ...
        $apikey = get_default($PIVOTX['config']->get('googlemaps_apikey'), $googlemaps_config['googlemaps_apikey']);
...
EOF;
    ...
    if (isset($params['width'])) {
        ...
        $width = get_default($PIVOTX['config']->get('googlemaps_width'), $googlemaps_config['googlemaps_width']);
    }
    if (isset($params['height'])) {
        $height = $params['height'];
    } else {
        $height = get_default($PIVOTX['config']->get('googlemaps_height'), $googlemaps_config['googlemaps_height']);
    }
    if (isset($params['zoom'])) {
        $zoom = $params['zoom'];
    } else {
        $zoom = get_default($PIVOTX['config']->get('googlemaps_zoom'), $googlemaps_config['googlemaps_zoom']);
    }
    if (isset($params['lat'])) {
        $lat = $params['lat'];
    } else {
        $lat = get_default($PIVOTX['config']->get('googlemaps_lat'), $googlemaps_config['googlemaps_lat']);
    }
    if (isset($params['long'])) {
        $long = $params['long'];
    } else {
        $long = get_default($PIVOTX['config']->get('googlemaps_long'), $googlemaps_config['googlemaps_long']);
    }

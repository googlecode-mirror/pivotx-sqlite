...
//<![CDATA[
tinyMCE_GZ.init({
    plugins : "safari,table,searchreplace,paste,xhtmlxtras,media,fullscreen", // add contextmenu if you don't want Firefox's spellchecker
	themes : 'advanced',
	languages : 'en',
	disk_cache : true,  // FIXME: select the right language. old one is pre_editor_wysi.php
	debug : false
}, function() {
    tinyMCE.init({
        theme : "advanced",
        language : "en", // FIXME: select the right language. old one is pre_editor_wysi.php
        mode : "exact",
        ...
        extended_valid_elements : "a[href|target|name|title|rel|class|id],hr[class|width|size|noshade],font[face|size|color|style|class]," +
                "span[class|align|style],img[class|src|border|alt|title|hspace|vspace|width|height|align|onmouseover|onmouseout|name],"+
        ...
        theme_advanced_buttons1 : "undo,redo,pastetext,removeformat,fullscreen,|,bold,italic,underline,strikethrough,hr,bullist,numlist,|," + 
                "cite,acronym,abbr,|,link,unlink,|,pivotxImage,pivotxPopup,pivotxDownload,pivotxTag,|,forecolor,backcolor",
        theme_advanced_buttons2 : "formatselect,outdent,indent,justifyleft,justifycenter,justifyright,|," + 
                ...
                /* recently removed: pasteword, sub,sup, image, anchor */
        ...
        media_strict : true,
        setup : function(ed) {
            ed.addButton('pivotxImage', { title:'[[t]]Insert PivotX Image[[/t]]', image:'editor_wysi/themes/advanced/img/pivotx_image.gif', onclick:function() { openImageWindow(this.id); }  });
            ed.addButton('pivotxPopup', { title:'[[t]]Insert PivotX Popup[[/t]]', image:'editor_wysi/themes/advanced/img/pivotx_popup.gif', onclick:function() { openImagePopupWindow(this.id); }  });
            ed.addButton('pivotxDownload', { title:'[[t]]Insert PivotX Download[[/t]]', image:'editor_wysi/themes/advanced/img/pivotx_download.gif', onclick:function() { openDownloadWindow(this.id); }  });
            ed.addButton('pivotxTag', { title:'[[t]]Insert PivotX Tag[[/t]]', image:'editor_wysi/themes/advanced/img/pivotx_tag.gif', onclick:function() { openTagWindow(this.id); }  });
        }    
    });
...
function doImage(image_name, image_alt, image_align, name) {

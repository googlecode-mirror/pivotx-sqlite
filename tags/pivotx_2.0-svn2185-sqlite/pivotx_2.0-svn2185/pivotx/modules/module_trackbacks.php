...
function handlePostTrackback($uri,$date) {
    ...
    if (ip_check_block($_SERVER['REMOTE_ADDR'])) {
    ...
    if ($PIVOTX['config']->get('allow_html_in_comments')==1) {
        $_POST['excerpt'] = strip_tags_attributes($_POST['excerpt'], "*");
    } else {
        $_POST['excerpt'] = strip_tags_attributes($_POST['excerpt'], "<b><em><i><strong>");
    }
    ...
    if ($PIVOTX['config']->get('maxhrefs') > 0) {
        $low_excerpt = strtolower(trackback_format($orig_excerpt));
...
function editTrackback($entry,$track_key,$track_val) {
    ...
    if ($PIVOTX['db']->db_type == "flat") {
        ...
        if (file_exists($lasttrackfile)) {
            $lasttrack = load_serialize($lasttrackfile, true, true);
        } else {
            $lasttrack = array();
        }
        ...
        save_serialize($lasttrackfile, $lasttrack );
    }
...
function getTracbackKeyJS($uri, $date) {
    ...
    $tburl = $PIVOTX['paths']['host'] . makeFilelink($PIVOTX['db']->entry['code'], '', '');

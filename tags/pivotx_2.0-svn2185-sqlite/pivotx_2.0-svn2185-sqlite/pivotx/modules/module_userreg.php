...
//
// $Id: module_userreg.php 2115 2009-09-07 12:48:44Z hansfn $
//
// ---------------------------------------------------------------------------
    ...
    function login() {
        ...
        if ($logged_in) {
            setcookie('piv_reguser', stripslashes($user['name'].'|'.md5($user['pass'])),
                time()+2592000, $PIVOTX['paths']['site_url']);
...
EOM;
        ...
        $trans['pass_reset_desc'] = __('If you\'ve forgotten your password, enter your username and PivotX will send you an e-mail with a link to reset your password.');
        $trans['back_login'] = __('Back to login');
        $trans['register_new'] = __('Register a new username.');
        foreach ($trans as $key => $val) {
            $form = str_replace("%".$key."%", $val, $form);
    ...
    function submitReg() {
        ...
        if ($this->isUser($input['name'])) {
            ...
            $this->input['message'] .= __('E-mail address is already taken... Please pick another name.');
            $text = $this->showRegPage();
    ...
    function delUser($name = '') {
        ...
        if ($name == '') { 
            setcookie("piv_reguser", "", time()-1000, $PIVOTX['paths']['site_url']);
    ...
    function weblogLinks() {
        ...
        foreach ($our_weblogs as $weblogkey => $weblog) {
            $text .= sprintf("<p>&laquo; ".__('Back to')." <a href=\"%s\">%s</a></p>", 
                sprintf("%sindex.php?w=%s", $PIVOTX['paths']['site_url'], $weblogkey), $weblog['name'] );
        }
    ...
    function regUser($user) {
        ...
        if (save_serialize($PIVOTX['paths']['db_path'].'users/'.$name_md5.'.php', $user)) { 
    ...
    function loadUser($name) {
        ...
        if ($this->isUser($name)) {
            $user = load_serialize($PIVOTX['paths']['db_path'].'users/'.$name_md5.'.php',true);
            return $user;
        } else {
            return FALSE;
        }

...
//
// $Id: module_comments.php 2185 2009-10-22 15:09:06Z pivotlog $
//
// ---------------------------------------------------------------------------
...
function handlePostComment() {
    ...
    if ($PIVOTX['config']->get('allow_html_in_comments')==1) {
        $_POST['piv_comment'] = strip_tags_attributes($_POST['piv_comment'], "*");
    } else {
        $_POST['piv_comment'] = strip_tags_attributes($_POST['piv_comment'], "<b><em><i><strong>");
    }

    // Do some more processing on the comment itself: trimming, standardizing line-breaks.
    $comment_text = strip_trailing_space($_POST['piv_comment']);
    ...
    if ($PIVOTX['config']->get('maxhrefs') > 0) {
        ...
        $low_comment_formatted = strtolower(comment_format($temp_comment['comment']));
...
function generateModerationQueue($tempcomm) {
    ...
    if (file_exists($modqueue_file)) {
        $modqueue = load_serialize($modqueue_file, true, true);
    } else {
        $modqueue = array();
    }
    ...
    save_serialize($modqueue_file, $modqueue );
...
function getModerationQueue() {
    ...
    if ($PIVOTX['db']->db_type == "sql") {
        ...
        $PIVOTX['db']->db_lowlevel->sql->query("SELECT co.*, e.title AS entrytitle
    	...
    	if (file_exists($modqueue_file)) {
    		$modqueue = array_reverse(load_serialize($modqueue_file, true, true));
    	}
...
function editComment($entry,$comm_key,$comm_val) {
    ...
    if ($PIVOTX['db']->db_type == "flat") {
        ...
        if (file_exists($lastcommfile)) {
            $lastcomm = load_serialize($lastcommfile, true, true);
        } else {
            $lastcomm = array();
        }
        ...
        save_serialize($lastcommfile, $lastcomm );
    }
...
function sendMailComment($temp_comment, $notifications='') {
    ...
    if (count($addr_arr)>0) {
        ...
        $id = safe_string($temp_comment["name"],TRUE) . "-" .  
            format_date($temp_comment["date"], "%ye%%month%%day%%hour24%%minute%");
...
function rememberCommentInfo($comment) {
    ...
    $duration = time() + (30 * 24 * 3600); // About one month from now..
    ...
    setcookie('pivotcomment', $cookievalue, $duration, $path, $domain);
    
    // Also set a cookie for the spamquiz answer, while we're at it..
    if (!empty($_POST['spamquiz_answer'])) {
        setcookie('spamquiz_answer', $_POST['spamquiz_answer'], $duration, $path, $domain);    
    }
    // debug("setcookie('pivotcomment', $cookievalue, $duration, $path, $domain) ");

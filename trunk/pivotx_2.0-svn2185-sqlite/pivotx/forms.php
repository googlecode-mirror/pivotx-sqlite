...
//
// $Id: forms.php 2149 2009-09-24 19:19:19Z hansfn $
//
// ---------------------------------------------------------------------------
    ...
    if ($template=="normal") {
        ...
        $form->add( array(
            ...
            'text' => makeJtip(__('Reset my password'), 
                __('If you\'ve forgotten your password, check the box, enter your username and PivotX will send you an e-mail with a link to reset your password.')),
        ));
...
function getBackupDatabaseForm() {
    ...
    $form = new Form("backupdatabaseform", "", __("Backup entries database"));
...
function getSetupUserForm() {
    ...
    if (sizeof($supportedDB)) {
          ...
          $form->add( array(
              ...
              'error' => __('Error'),
              'size' => 30,
              'validation' => '', // ifany|string|minlen=2|maxlen=20
          ...
          $form->add( array(
              ...
              'error' => __('Error'),
              'size' => 30,
              'validation' => '', // ifany|string|minlen=2|maxlen=20 (we have to allow for 'empty passwords' with MySQL..
          ...
          $form->add( array(
              ...
              'error' => __('Error'),
              'size' => 30,
              'validation' => 'ifany|string|minlen=2|maxlen=50',
          ...
          $form->add( array(
              ...
              'error' => __('Error'),
              'size' => 30,
              'validation' => 'ifany|string|minlen=2|maxlen=30',
              'text' => makeJtip(__('Table Prefix'), __('The prefix to use for the database tables. By changing this, you can run multiple installations of pivot from one MySQL database. If you don\'t intend to do so (yet), just leave this set to "pivotx_".'))
          ));
...
EOM;
    ...
    $form->add( array(
        ...
        'isrequired' => 1,
        'validation' => 'any',
        'text' => makeJtip(__('Userlevel'), __('Userlevel will determine what kind of actions this user can perfom in pivot.')),
    ));
...
function getEditVisitorForm() {
    ...
    // We only allow changing the e-mail, verified and disabled status.
...
EOM;
    ...
    $form->add( array(
        ...
        'size' => 10,
        'isrequired' => 1,
        'validation' => 'string|integer|min=1|max=999999'
    ));
...
EOM;
    ...
    foreach($themes as $themename) {
        ...
        $theme = load_serialize($themename);
...
EOM;
    ...
    $form->add( array(
        ...
        'size' => 30,
        'validation' => 'ifany|string|minlen=2|maxlen=80',
        'text' => makeJtip(__('URL to Weblog'), __('PivotX will determine the url of your weblog if you leave this field blank. If you use your weblog as part of a frameset, or as a serverside include, you can use this to override it.'))
    ));
...
EOM;
    ...
    foreach ($subweblogs as $key) {
        ...
        $form->add( array(
            'type' => 'custom',
            'text' => "<tr><td colspan='2'><h3>Subweblog $key</h3></td></tr>"
...
EOM;
    ...
    $form->add( array(
        ...
        'size' => 50,
        'isrequired' => 1,
        'validation' => 'ifany|string|minlen=4|maxlen=30',
        'text' => makeJtip(__('RSS URL'), __('If you use a service like FeedBurner, you can give the link to your FeedBurner feed here. The link will be used in the RSS badge and in the autodiscovery link on this weblog. Leave this blank to use Pivot\'s Feed, or specify a full URL.'))
    ));
    ...
    $form->add( array(
        ...
        'isrequired' => 1,
        'validation' => 'ifany|string|minlen=4|maxlen=30',
        'text' => makeJtip(__('Atom URL'), __('If you use a service like FeedBurner, you can give the link to your FeedBurner feed here. The link will be used in the Atom badge and in the autodiscovery link on this weblog. Leave this blank to use Pivot\'s Feed, or specify a full URL.'))
    ));
    ...
    $form->add( array(
        ...
        'size' => 50,
        'isrequired' => 1,
        'validation' => 'string|minlen=4|maxlen=50',
...
EOM;
    ...
    */
    ...
    /* -- Commented out until Trackbacks are fully implemented
    $form->add( array(
        'type' => 'custom',
        'text' => "<tr><td colspan='2'><h3>".__('Trackback Settings')."</h3></td></tr>"
    ...
    $form->add( array(
        'type' => 'textarea',
        'name' => 'trackback_format',
        'label' => __('Format of Trackbacks'),
        'text' => makeJtip(__('Format of Trackbacks'), __('This specifies the formatting of trackbacks on the entry pages.')),
        'error' => __('Error'),
        'rows' => 5,
        'cols' => 60,
        'isrequired' => 1,
    ...
    $form->add( array(
        'type' => 'textarea',
        'name' => 'trackback_link_format',
        'label' => __('Format of Trackback Link'),
        'text' => makeJtip(__('Format of Trackback Link'), __('This specifies the formatting of the trackback link on the entry pages.)')),
        'error' => __('Error'),
        'rows' => 5,
        'cols' => 60,
        'isrequired' => 1,
...
EOM;
    ...
    $form->add( array(
        ...
        'size' => 50,
        'isrequired' => 1,
        'validation' => 'string|minlen=2|maxlen=30'
    ));


    // Make the list of weblogs and pages..
    $root_options = array();
...
*/
    ...
    $form->add( array(
        ...
        'value' => '',
        'text' => makeJtip(__('Use Mod_rewrite'), __('If you use Apache\'s Mod_rewrite option, PivotX will make urls like www.example.org/archive/2008/05/30/nice_weather, instead of www.mysite.com/?e=134. Not all servers support this, so please read the <a href="http://docs.pivotx.net/doku.php?id=using_apache_s_mod_rewrite">Using Apache\'s Mod_rewrite</a> section in the manual about this.')),
        'error' => __('Error'),
        'options' => $options
    ));
...
EOM;
    ...
    $form->add( array(
        ...
        'size' => 6,
        'isrequired' => 1,
        'validation' => 'integer|minlen=1|maxlen=4'
    ));
...
EOM;
    ...
    $form->add( array(
        ...
        'label' => __('Debug mode'),
        'value' => '',
        'text' => makeJtip(__('Debug mode'), __('Show random debug information, every here and there..'))
    ));
...
/*
    $form->add( array(
        ...
        'label' => __('Default Extension'),
        'text' => makeJtip(__('Default Extension'), ''),
        'value' => '',
        'error' => __('Error'),
        'size' => 12,
        'isrequired' => 1,
...
*/
    ...
    $form->add( array(
        ...
        'size' => 12,
        'isrequired' => 1,
        'validation' => 'integer|minlen=1|maxlen=8'
    ));
...
EOM;
    ...
    $form->add( array(
        ...
        'error' => __('Error'),
        'size' => 50,
        'validation' => 'ifany|string|minlen=1|maxlen=100'
    ));
...
*/
    ...
    $form->add( array(
        ...
        'value' => '',
        'size' => 60,
        'validation' => 'ifany|string|minlen=1|maxlen=500'
    ));
...
function getExtensionsForm($list) {
    ...
    $form->html['start'] = <<< EOM
<form  enctype='multipart/form-data'  name='%name%' id='%name%' action="%action%" method='post'>
<table border='0' cellspacing='0' cellpadding='4' class='formclass' style="border-width: 0px !important;" width="600">
EOM;
    ...
    foreach ($list as $type => $extensionslist) {
        ...
        $form->add( array(
            'type' => "custom",
            'text' => "<tr><td colspan='3'><h3 style='margin: 6px 0 0 0;'>$title</h3></td></tr>\n"
        ));

        foreach ($extensionslist as $extension) {

            $description = sprintf("<p style='margin: 0 0 6px 0;'><strong>%s</strong><br />%s</p><p style='color: #999; margin: 0; font-size: 11px;'>%s: %s",
                $extension['extension'],
            ...
            // a posted form.
            if (!empty($_POST)) {
                $value = $_POST[$extension['name']];
            } else {
                $value = in_array($extension['name'], $PIVOTX['extensions']->getActivated()) ? 1 : 0;
            }
...
function makeJtip($caption, $str) {
    ...
    $str = str_replace("'", "&#39;", $str);
    $id = "tip_".safe_string($caption, true);

...
function renderTemplate($template, $page="") {
    ...
    // Open links to the front-end in new windows/tabs unless the user 
    // has configured PivotX to open them in the same window. 
    if (!$PIVOTX['config']->get('front_end_links_same_window')) {
        $jscode = <<<EOM
<script type="text/javascript">
    jQuery(document).ready(function(){
        $("a.front_end").attr('target','_blank');
    });
</script>
EOM;
        $html = str_replace("</body>", $jscode."\n</body>", $html);
    }

...
//
// $Id: module_moblog.php 2825 2010-07-07 21:41:01Z hansfn $
//
// ---------------------------------------------------------------------------
...
class Moblog {
    ...
    function compose_entry() {
        ...
        if ((strlen($entry['introduction'])>2) || (strlen($entry['body'])>2)) {
            ...
            $msg = sprintf(__("Your entry has been posted! \n\nTitle: %s\nUser: %s\nCategory: %s\nIntroduction: %s"),
               $entry['title'], $entry['user'], implode("", $entry['category']), $entry['introduction']);

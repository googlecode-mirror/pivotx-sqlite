...
//
// $Id: lib.php 2176 2009-10-19 22:31:04Z hansfn $
//
// ---------------------------------------------------------------------------
...
$version = "2.1.0";
$codename = "SVN - Pre-alpha"; 
$svnrevision = '$Rev: 2176 $';

$minrequiredphp = "4.3.0";
$minrequiredmysql = "4.1";
$dbversion = "5"; // Used to track if it's necessary to upgrade the DB.
...
/**
 * Checks whether Pivot is installed/setup.
 *
 * @return boolean
 */
...
function displayPage() {
    ...
    if ( ( $PIVOTX['session']->isLoggedIn() === false) || ($PIVOTX['users']->count()==0) ) {
        ...
        if(!in_array($_GET['page'], array('bookmarklet'))) {
            $_GET['page']="login";
        }
    }
    ...
    if ($_GET['page'] == "") {
        ...
        $function = 'page' . ucfirst(strtolower(safe_string($_GET['page'])));
        $PIVOTX['template']->assign('currentpage', strtolower(safe_string($_GET['page'])));
    ...
    if (function_exists($function)) {
        ...
        $PIVOTX['messages']->addMessage("The page '<tt>".htmlspecialchars($_GET['page'])."</tt>' does not exist.");
...
function renderTemplate($template, $page="") {
    ...
    if (isset($PIVOTX['config'])) {
        ...
        $PIVOTX['template']->assign('now', format_date('', $PIVOTX['config']->get('fulldate_format')));
    ...
    // Send XML templates with the correct mime-type.
...
/**
 * For some reason, I can't get Smarty's clear_cache() to work the way i like it,
 * so we/re going to do it ourselves. This function crawls the pivotx/db/cache
 * folder, removing all files with $name in the filename.
 *
 * @param string $name
 */
function clearSmartyCache($name) {
    global $PIVOTX;
    
    $name = basename($name);
    
    // if $name is too short, refuse to do anything..
    if (strlen($name)<2) {
        return;     
    }
...
function setPaths($sites_path = ''){
    ...
    $PIVOTX['paths']['site_url'] = fixPathSlash(str_replace('\\', '/', dirname($PIVOTX['paths']['pivotx_url'])));
    ...
    if (empty($sites_path)) {
        ...
        $PIVOTX['paths']['home_urlh'] = $PIVOTX['paths']['pivotx_url'] . $sites_path;
    }
    ...
    $upload_base = $PIVOTX['config']->get('upload_path') ;
    if ($sites_path != '') {
    ...
    if (strpos($upload_base, "%")>0) {
        $PIVOTX['paths']['upload_base_path'] = fixPathSlash(dirname($PIVOTX['paths']['upload_path']));
        $PIVOTX['paths']['upload_base_url'] = fixPathSlash(dirname($PIVOTX['paths']['upload_url']));
    } else {
...
function sanitizePostedEntry($entry) {
    ...
    if ( (strlen($_POST['date1'])>7) && ($_POST['date1']!="00-00-0000") ) {
        ...
        $entry['date'] = fix_date($_POST['date1'], $_POST['date2']);
    } else {
    ...
    $entry['publish_date'] = fix_date($_POST['publish_date1'], $_POST['publish_date2']);
    $entry['title'] = strip_trailing_space($_POST['title']);
    $entry['subtitle'] = strip_trailing_space($_POST['subtitle']);

    $entry['introduction'] = strip_trailing_space($_POST['introduction']);
    $entry['introduction'] = tidy_html($entry['introduction'], TRUE);

    $entry['body'] = strip_trailing_space($_POST['body']);
    ...
    // Make sure we add all 'extra fields'
...
function sanitizePostedPage($page) {
    ...
    if ( (strlen($_POST['date1'])>7) && ($_POST['date1']!="00-00-0000") ) {
        // if the date is filled, we try to parse it..
        $page['date'] = fix_date($_POST['date1'], $_POST['date2']);
    } else {
        // otherwise we'll just assume 'now'.
        $page['date'] = date("Y-m-d-H-i", get_current_date());
    }
    ...
    $page['publish_date'] = fix_date($_POST['publish_date1'], $_POST['publish_date2']);
    $page['title'] = strip_trailing_space($_POST['title']);
    $page['subtitle'] = strip_trailing_space($_POST['subtitle']);
    $page['template'] = strip_trailing_space($_POST['template']);

    $page['introduction'] = strip_trailing_space($_POST['introduction']);
    $page['introduction'] = tidy_html($page['introduction'], TRUE);

    $page['body'] = strip_trailing_space($_POST['body']);
    $page['body'] = tidy_html($page['body'], TRUE);
...
function getFiles($basepath, $additionalpath, $imageurl) {
    ...
    $amount = count($files);
    $stepping = get_default($PIVOTX['config']->get('media_paging_threshold'), 50);
    ...
    for ($i=0; $i<$pages; $i++) {
        ...
        $first = substr(safe_string($filenames[ $i*$stepping ]),0 ,5);
        $last = substr(safe_string($filenames[ $endofindex ]),0 ,5);
    ...
    $PIVOTX['template']->assign('imageurl', fixpath($imageurl."/".$additionalpath."/"));
...
function fileOperations($folder) {
    ...
    // See if we perhaps need to upload some files
    if (isset($_FILES) && count($_FILES)>0 ) {

        // Only for advanced users and higher..
        $PIVOTX['session']->minLevel(2);

        require_once($PIVOTX['paths']['pivotx_path'].'includes/fileupload-class.php');

        $uploader = new uploader();

        // OPTIONAL: set the max filesize of uploadable files in bytes
        $uploader->max_filesize($PIVOTX['config']->get('max_filesize'));

        if ($_POST['additionalpath']!="") {
            $_POST['additionalpath'] = str_replace("../", "", $_POST['additionalpath']);
            $folder = $folder . fixpath($_POST['additionalpath'] . "/");
        }
        ...
        foreach($_FILES as $key => $file) {
            ...
            if ($file['name']!="") {
                ...
                if ($uploader->upload($key, $PIVOTX['config']->get('upload_accept'), '')) {
                    $success = $uploader->save_file($folder, $PIVOTX['config']->get('upload_save_mode'), 1);
                }

                if ($success) {
                    $msg = __('Successfully uploaded %s');
                    require_once($PIVOTX['paths']['pivotx_path'].'modules/module_imagefunctions.php');
                    if (auto_thumbnail($uploader->file['name'], $folder)) {
                       $msg .= ' ' . __('and created thumbnail');
                    } else {
                       debug("Failed to auto-create thumbnail for " . $uploader->file['name']);
                    }
                    $PIVOTX['messages']->addMessage(sprintf($msg, $uploader->file['name']));
                } else {
                    $msg = __('Failed to upload %s');
                    $PIVOTX['messages']->addMessage(sprintf($msg, $uploader->file['name']));
                }
        ...
        $_GET['additionalpath'] = $_POST['additionalpath'];
...
function templateList($dir="", $recursive=false) {
    ...
    while (false !== ($entry = $d->read())) {
        ...
        if (is_dir($dir.$entry)) {
            ...
            if (in_array($ext, array('html', 'htm', 'tpl')) && (substr($entry,0,1)!=".")) {
                $templates[] = $dir.$entry;
            }
...
function para_weblog_needed($weblog, $categories = "") {
    ...
    if (count($weblognames)>1) {
        if (($weblog != $weblog_default) || $PIVOTX['config']->get('para_weblog_always')) {
            return true;
        } else {
            return false;
        }
...
function liberal_unserialize($filename) {
    ...
    // Strip the 'pivot lamer protection'..
 ...
 * Gets called by liberal_unserialize() and (recursively) _lu_getarray()
 *
 * @param string $str
 */
...
/**
 * Clean smarty params: strip quotes, remove trailing spaces..
 *
 * @param array $params
 * @return array
 */
function clean_params($params) {
    ...
    foreach ($params as $key=>$param) {
        ...
        if (is_array($param)) {
            $params[$key] = clean_params($params[$key]);
...
function implode_deep($glue, $value) {
    ...
    if (is_array($value)) {
        ...
        foreach($value as $key=>$val) {
            if (is_array($value)) {
                $value[$key] = implode_deep($glue, $val);  
            }
        }
...
function tidy_html($text, $thorough=FALSE) {
    ...
    $text = preg_replace_callback("/<p>(\s*)<div(.*)>(.*)<\/div>(\s*)<\/p>/sUi", "tidy_html_callback_nesteddivs", $text);
    $text = preg_replace_callback("/<p>(\s*)<object(.*)>(.*)<\/object>(\s*)<\/p>/sUi", "tidy_html_callback_nestedobjects", $text);
 ...
 * @return string
 */
function tidy_html_callback_nestedobjects($match) {
...
function makeArchiveName($date='', $this_weblog='', $archive_unit='month') {
    ...
    $archive_num = (($archive_unit=="week")  ? format_date($date, "%weeknum%") :
                   (($archive_unit=="month") ? format_date($date, "%month%")   :  ''  ));
 ...
 * @return void
 */
function adddirtozip($zipfile,$dirname) {
    $d = dir($dirname);
    while (false !== ($entry = $d->read())) {
        if ( ($entry != ".") && ($entry != "..") ) {
...
function pivotxMail($to , $subject , $message, $headers='', $additional_parameters='') {
    ...
    $utf8_headers .= "\n$headers";
    $subject_prefix = get_default( $PIVOTX['config']->get('email_subject_prefix'), 
...
function format_date( $date="", $format="", $title="") {
    ...
    if ($date=="" || $date=="now") {
        $date= date("Y-m-d-H-i-s", get_current_date());
    }
 ...
 */
function entify($i, $encoding = 'utf-8') {
    if ($encoding == 'utf-8') {
        return i18n_entify($i);
    } else {
        debug('PivotX uses only UTF-8 - not entified');
        return $i;
    }
}
 ...
 */
function unentify($i, $encoding = 'utf-8') {
    if ($encoding == 'utf-8') {
        return i18n_unentify($i);
    } else {
        debug('PivotX uses only UTF-8 - not unentified');
        return $i;
    }
}
...
function encodemail_link($mail, $text, $title="", $encrypt=false) {
    ...
    if (!isemail($mail)) {
        debug("Provided e-mail address isn't valid");
        return $text;
    }
 ...
 * @return string
 */
function pivotx_markdown($str, $with_smartypants=0) {
    global $PIVOTX;
 ...
 * @return string
 */
function get_editlink($name, $uid, $prefix, $postfix, $type="entry") {
    global $PIVOTX;
...
function comment_format( $text, $striplinebreaks=false ) {
	...
	if ($PIVOTX['config']->get('allow_html_in_comments')==1) {
	    ...
	    $text = strip_tags_attributes($text, "<b><em><i><strong>");	
	}
...
function safe_string($str, $strict=false, $extrachars="") {
    ...
    $str = trim(strtr($str, "\xA1\xAA\xBA\xBF\xC0\xC1\xC2\xC3\xC5\xC7\xC8\xC9\xCA\xCB\xCC\xCD".
    ...
    $str = preg_replace('/'.preg_quote($regex).'/', "", $str);
 ...
 * safe_string, but using hyphens instead of underscores.
 *
 * @param string $str
 * @param string $type
 * @return string
 ...
 * @return string
 */
function makeExcerpt($str, $length=180) {
    global $PIVOTX;
    ...
    // Ensure that the excerpt is parsed.
    $str = parse_intro_or_body($str);

    $from = array("&quot;", "\n", "\r", "  ");
    $to = array('"', " ", "", " ");

    $excerpt = strip_tags(@html_entity_decode($str, ENT_NOQUOTES, 'UTF-8'));
    $excerpt = str_replace($from, $to, $excerpt);
    $excerpt = trimtext($excerpt, $length, false, false);

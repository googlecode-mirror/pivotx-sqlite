...
//
// $Id: module_moblog.php 2056 2009-08-23 17:15:38Z hansfn $
//
// ---------------------------------------------------------------------------
...
class Moblog {
    ...
    function Moblog() {
        ...
        $cfg['pop_port'] = get_default($PIVOTX['config']->get('moblog_pop_port'), 110);
        $cfg['imap_mailbox'] = get_default($PIVOTX['config']->get('moblog_imap_mailbox'),'' );
        $cfg['spam_category'] = get_default($PIVOTX['config']->get('moblog_spam_category'), 'spam');
        $cfg['replyaddress'] = get_default($PIVOTX['config']->get('moblog_replyaddress'), '');
        $cfg['title'] = get_default($PIVOTX['config']->get('moblog_title'), "Moblog on " . date("m-d H:i"));
        $cfg['status'] = get_default($PIVOTX['config']->get('moblog_status'), 'publish');
        $cfg['allow_comments'] = get_default($PIVOTX['config']->get('moblog_allow_comments'), 1);
        $cfg['save_mail'] = get_default($PIVOTX['config']->get('moblog_save_mail'), false);
        $cfg['leave_on_server'] = get_default($PIVOTX['config']->get('moblog_leave_on_server'), false);
        $cfg['send_confirmation'] = get_default($PIVOTX['config']->get('moblog_send_confirmation'), true);
        $cfg['skip_thumbnail'] = get_default($PIVOTX['config']->get('moblog_skip_thumbnail'), false);
        $cfg['maxwidth'] = get_default($PIVOTX['config']->get('moblog_maxwidth'), 400);
        $cfg['maxheight'] = get_default($PIVOTX['config']->get('moblog_maxheight'), 200);
        $cfg['quality'] = get_default($PIVOTX['config']->get('moblog_quality'), 60);
        $cfg['click_for_image'] = get_default($PIVOTX['config']->get('moblog_click_for_image'), __('Click for image'));
    ...
    function compose_entry() {
        ...
        if ((strlen($entry['introduction'])>2) || (strlen($entry['body'])>2)) {
            ...
            $entry['code']=">";
            $entry['date'] = $entry['publish_date'] = date('Y-m-d-H-i');
        ...
        $msg.= "\n\nprocessed: ". date("dS \o\f F Y H:i:s")."\n";
    ...
    function parse_body($part) {
        ...
        // Convert body to UTF-8 if the e-mail isn't using UTF-8 as charset.
        ...
        if ( ($filename !="") && ( ($ext=="jpg") || ($ext=="jpeg") ||  ($ext=="gif") ||  ($ext=="png") )) {
            ...
            if ( ($mywidth > $this->cfg['maxwidth']) || ($myheight > $this->cfg['maxheight'])) {
                ...
                if (strlen($thumbfile)>2) {
                    ...
                    $entry['introduction'] .="\n[[popup:$filename:".$this->cfg['click_for_image']."::center:1]]\n";
                }
    ...
    function parse_download($part) {
        ...
        $this->entry['introduction'] .="[[download:$filename:icon:$filename:]]";

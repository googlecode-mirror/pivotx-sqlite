...
// - Updatecheck: http://www.pivotx.net/update.php?ext=picnik
// - Description: An extension to allow editing images trough picnik.com.
// - Date: 2008-02-10
// - Identifier: picnik
...
function picnikHookMediaBefore() {
    ...
    $hide = get_default($PIVOTX['config']->get('picnik_replacebutton'), $picnik_config['picnik_replacebutton'], true);
    if ($hide==true) {
        $PIVOTX['extensions']->hide('medialineimage');
    }

    $apikey = get_default($PIVOTX['config']->get('picnik_apikey'), $picnik_config['picnik_apikey']);
...
function picnikHookMediaImage($value) {
    ...
    $output = sprintf('<a href="#" onclick="return openPicnik(\'%s\', \'%s\');">
        <img src="extensions/picnik/picnik.png" alt="" />Edit in Picnik</a>',
...
function picnikAdmin(&$form_html) {
    ...
    $form->add( array(
        ...
        'label' => "Keep original",
        'text' => "Yes, Keep the original image after editing it in Picnik"
    ));
    ...
    $form->add( array(
        ...
        'label' => "Hide buttons",
        'text' => "Yes, replace the original 'edit' button for images"
    ));
    ...
    $form->add( array(
        'type' => 'text',
        'name' => 'picnik_apikey',
        'label' => "Picnik API key",
        'value' => '',
        'error' => 'That\'s not a proper API key!',
        'text' => "If you have your own picnik API key, you may enter it here. <br />
    ...
    if ( $host=="localhost" || (substr($host,0,5)=="10.0.") || (substr($host,0,6)=="127.0.")  || (substr($host,0,8)=="192.168.") ) {
        ...
        $form->add( array(
            'type' => 'info',
            'text' => "<div class='warning'><p><strong>Note:</strong> It seems you're running Pivot from a local network. The Picnik extension<br />
                 needs to be able to fetch the images over the internet, so it might not work on<br />  your current setup.</p></div>"
        ));
